            ...
            <div className="container-fluid d-flex justify-content-center gap-4 flex-wrap">
                {cart.map((element) => (
                    <div key={element.id} className="card overflow-hidden" style={{ maxWidth: "345px", maxHeight: "370px" }}>
                        <img src={`../../../public/${element.imageId}`} className="card-img-top img-fluid h-50 p-1 border-bottom" alt={element.title} />
                        <h5 className="card-title text-center">{element.title}</h5>
                        <p className=" ms-2 fw-bold">${totalProduct(element)}</p>
                        <div className="d-flex gap-3 justify-content-center align-items-center">
                            <button className="btn btn-danger btn-circle" onClick={() => decrementItem(element, 1)}>-</button>
                            <p className="fw-bold quantity">{element.cantidad}</p>
                            <button className="btn btn-success btn-circle" onClick={() => addItem(element, 1)}>+</button>
                        </div>
